...
const ZONES = [
  { name: "몬트리올 다운타운/올드몬트리올", cityName: "montreal", bbox: [45.4900, -73.5900, 45.5200, -73.5400] as [number, number, number, number] },
  { name: "플라퇴/마일엔드", cityName: "montreal", bbox: [45.5100, -73.6200, 45.5400, -73.5700] as [number, number, number, number] },
  { name: "생로랑/로즈몬", cityName: "montreal", bbox: [45.5200, -73.6600, 45.5700, -73.5800] as [number, number, number, number] },
  { name: "노트르담드그레스/웨스트몬", cityName: "montreal", bbox: [45.4700, -73.6200, 45.5100, -73.5800] as [number, number, number, number] },
  { name: "라발", cityName: "montreal", bbox: [45.5500, -73.7800, 45.6200, -73.6700] as [number, number, number, number] },
];
...
const SHOP = ["convenience", "supermarket", "mall", "clothes", "electronics", "beauty", "hairdresser", "asian", "korean", "japanese", "grocery", "seafood"];
...
async function main() {
  console.log("🗺️  Montreal OSM 수집 시작\n");
  console.time("총 소요");
  ...
  const out = path.join(outDir, `montreal-osm-full-${date}.json`);
